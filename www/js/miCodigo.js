...
function btnRegistroUsuarioHandler() {
    ...
    if (nombreIngresado && apellidoIngresado && direcciónIngresado && emailIngresado && passwordIngresado && verificacionPasswordIngresado) {
      if (passwordIngresado === verificacionPasswordIngresado) {
        ...
        .then((respuestaBody)=>{
          if (respuestaBody.error) document.querySelector("#pRegistro").innerHTML = respuestaBody.error;                
        ...
        .catch((error) => {
          ...
          document.querySelector("#pRegistro").innerHTML = "Ya existe un usuario con ese email.";
      }
    } else {
